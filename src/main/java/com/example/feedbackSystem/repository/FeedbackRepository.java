...
public interface FeedbackRepository extends JpaRepository<Feedback, Long> {
    ...
    // The previous query caused a SQL error (1055, 42000) because 'f.performanceCycle.startDate'
    // was in the ORDER BY clause but not in the GROUP BY clause, which violates MySQL's
    // ONLY_FULL_GROUP_BY SQL mode.
    //
    // Solution: Add f.performanceCycle.startDate to the GROUP BY clause.
    // This ensures that each unique combination of title and start date forms a group,
